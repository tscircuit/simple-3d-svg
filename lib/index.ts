...
function proj(p: Point3, w: number, h: number): Proj | null {
	...
	for (const box of scene.boxes) {
		...
		for (const idx of FACES) {
			...
			faces.push({
				pts: p4,
				depth: zSum / 4,
				fill: `rgba(${box.color[0]},${box.color[1]},${box.color[2]},${box.color[3]})`,
			});
		}
			...
			if (pts.every(Boolean)) {
				...
				if (lu && lv) {
					...
					const cx = pts.reduce((s, p) => s + (p as Proj).x, 0) / 4,
						cy = pts.reduce((s, p) => s + (p as Proj).y, 0) / 4,
						cz = vc[TOP[0]].z;
					...
					labels.push({
						...
						depth: cz,
						text: box.topLabel,
						fill: `rgba(${fillCol[0]},${fillCol[1]},${fillCol[2]},${fillCol[3]})`,
					});
				}
			}
		}
